...
void ImagDisp(UART_HandleTypeDef *huart)
{
	// uint8_t data[3];
	// char d[2];
	...
	for(i = 0; i < 240; i++)
	{
		for(j = 0; j < 320; j++)
		{
			...
			if (i%8 == 0 && j%8 == 0)
			{
				...
				if (greyscale == 1 || greyscale == 2 || greyscale == 3 || greyscale == 4 || greyscale == 5 || greyscale == 6 || greyscale == 7 || greyscale == 8)
				{
						greyscale = 0;
				}
				...
				grey[i*5+j/8+1] = greyscale;
			}
		
		}
		
	}
	HAL_UART_Transmit(huart, grey, sizeof(grey), HAL_MAX_DELAY);
	//HAL_Delay(500);
	HAL_Delay(2000);
